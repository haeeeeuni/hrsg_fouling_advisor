...
- 컬럼별 결측률 표
- **구조 검증(4.1) 오류가 1건이라도 있으면 적재 버튼 비활성화.** 경고만 있으면 적재 가능.
- 값 검증(4.2)의 행 단위 오류(`TIMESTAMP_PARSE_ERROR`, `FUTURE_TIMESTAMP`)는 **해당 행만 제외하고 적재는 진행**하며, 제외 건수를 리포트에 표시한다.

> **TODO(질문) / 해석 근거(2026-09-20, Phase 2):** 원문은 "오류가 1건이라도 있으면 적재 비활성화"였으나,
> 4.2 표에서 같은 '오류' 등급인 `TIMESTAMP_PARSE_ERROR` 의 처리가 "해당 행 제외"이고 `AC-03-2` 도
> "파싱 실패 행이 제외되고 건수가 리포트에 표시된다"를 요구해 서로 충돌했다.
> 가장 단순한 해석으로 **구조 오류만 차단**하도록 확정했다. 운영 정책이 다르면 이 항목을 먼저 고친다.
