...
> **검증 메모(2026-09-20, Phase 4):** 위 예시의 손실 출력·일일 손실 비용은 4.1~4.2 식과 3.1 기본 계수로
> **정확히 재현된다**(Δ차압 3.39 kPa, Δ스택온도 9.38 ℃ → 2.8 MW = GT 1.9 + ST 0.9, 5,712,000 원/일).
>
> 다만 예시의 정지 손실 `244,800,000 원`은 4.4 식에 정격 총출력 240 MW(GT 160 + ST 80)를 넣으면
...
4.6의 방법(평가 기간 내 누적 순편익 최대화)은 평가 기간이 고정이라 구조적으로
**권고일이 0보다 약간 뒤로 밀린다.** 늦게 세정할수록 기간 종료 시점의 잔류 오염
(= 세정 시점 FI × (1 − 회복률))이 낮아지기 때문이다.
오염이 심할수록 그 폭은 작아진다(기울기 0.18/일·FI 62 기준 D+9).
화면에는 권고 시점을 **3개 시나리오 비교표와 함께** 제시해 사용자가 판단하게 한다.
