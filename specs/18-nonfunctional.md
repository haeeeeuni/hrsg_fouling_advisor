...
M1 MacBook Pro / PostgreSQL 16(Homebrew) / Celery solo 풀 기준. 실측값이다.

| 항목 | 목표 | 실측 | 여유 |
|------|------|------|------|
| CSV 검증 + 적재 (1,052,640행 / 123MB) | < 5분 | **155.5초** | 2.0배 |
| └ 파일 전송 | | 0.6초 | |
| └ 검증 | | 30.3초 | |
| └ 적재 | | 124.5초 | |
| 분석 실행 (105,264행, 2년치 10분 주기) | < 60초 | **7.5초** | 8.0배 |
| PDF 리포트 생성 | < 30초 | **4.4초** | 6.8배 |
| 엑셀 리포트 생성 | < 20초 | **0.2초** | 100배 |
| 목록 API (페이지 50건) | < 500ms | **23~27ms** | 18배 |
| 대시보드 데이터 (6개 엔드포인트 합) | < 3초 | **0.16초** | 18배 |

- **적재 중 워커 최대 RSS 608MB.** `read_csv(chunksize=)` 스트리밍과
  `bulk_create(batch_size=5000)` 덕분에 파일 크기에 비례해 늘지 않는다(`specs/03` §5).
- 분석은 명세가 든 예(1년치 약 5만 행)의 **두 배 규모**로 측정했고도 8배 여유가 있다.
- 병목은 적재(124.5초)다. 100만 행 기준 초당 약 8,400행.
...
## 10. 수용 기준 (AC)
- [x] AC-18-1: 100만 행 CSV 적재가 5분 이내에 끝난다. (2026-09-21 실측 155.5초, §1 참조)
- [x] AC-18-2: `DEBUG=False` 운영 설정에서 정상 동작하고 보안 헤더가 응답에 포함된다. (2026-09-21 Compose 스택에서 확인)
- [x] AC-18-3: 로그에 비밀번호·전체 사번이 남지 않는다. (`common/tests/test_logging_filters.py` 6건)
- [x] AC-18-4: `seed_defaults`를 두 번 실행해도 데이터가 중복되지 않는다. (2026-09-21 확인)
- [x] AC-18-5: 엑셀 출력물에 CSV 인젝션 방지 처리가 적용된다. (`reports/tests/test_excel.py` 5건)
